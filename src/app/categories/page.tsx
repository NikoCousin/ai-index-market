...
export default function CategoriesIndexPage() {
  ...
  const getToolCount = (categorySlug: string) => {
    return data.tools.filter((t) => t.categories.includes(categorySlug)).length;
  };
  ...
  return (
    <main className="container mx-auto px-4 py-12">
      ...
      <div className="mb-12 text-center">
        {/* FIXED: Text is now White */}
        <h1 className="text-3xl font-bold tracking-tight text-white sm:text-4xl mb-4">
          Explore AI Categories
        </h1>
        {/* FIXED: Text is now Light Grey */}
        <p className="text-lg text-slate-400 max-w-2xl mx-auto">
          Browse our curated directory by topic. Find the perfect tool for your
          specific workflow.
        </p>
      </div>
        ...
        {data.categories.map((category: Category) => {
          ...
          return (
            <Link
              ...
              // FIXED: Card background is now Dark (bg-slate-900) and Border is Dark (border-slate-800)
            ...
            >
              ...
              <p className="text-slate-400 text-sm leading-relaxed">
                {category.description ||
                  `Browse the top ${category.name} tools.`}
              </p>
