...
export async function generateMetadata({ params }: Props) {
  ...
  const data = loadSeed();
  const useCase = data.useCases.find((c) => c.slug === slug);

  if (!useCase) return { title: "Use Case Not Found" };

  return {
    title: `Best AI Tools for ${useCase.name} | IndexMarket`,
    description: `Discover the top AI tools for ${useCase.name}.`,
  };
}
...
export default async function UseCasePage({ params }: Props) {
  ...
  // Filter tools that have this use case
    ...
    <main className="container mx-auto px-4 py-12 max-w-6xl">
      ...
      <div className="mb-8">
        <h1 className="text-3xl font-bold tracking-tight text-white sm:text-4xl mb-2">
          Top <span className="text-blue-400">{useCase.name}</span> AI Tools
        </h1>
        <p className="text-lg text-slate-400">
          Market cap and rankings for the top {useCase.name.toLowerCase()}{" "}
